...
import { calculateSignature, generateNonce, getCurrentTimestamp, } from './signature';
/**
 * 签名验证拦截器
 * 自动为所有请求添加签名验证头
 */
export function createSignatureInterceptor() {
    return (config) => {
        ...
        // 计算签名
        const signature = calculateSignature((config.method || 'GET').toUpperCase(), pathOnly, // 路径部分（不含查询参数）
        queryString || '', // 查询字符串
